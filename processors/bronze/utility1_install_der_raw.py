...
def loadSource():
    ...
    schema = StructType([
        ...
        , StructField("TotalChargesCESIR", DecimalType(38,4), nullable=True)
        , StructField("TotalChargesConstruction", DecimalType(38,4), nullable=True)
        , StructField("CESIR_EST", IntegerType(), nullable=True)
        , StructField("SystemUpgrade_EST", IntegerType(), nullable=True)
    ...
    file_path = "/Users/kateweber/dev/ESource-Case-Study/source/utility1_install_der/utility1_install_der.csv"

    transformSQL = """
            SELECT *
            FROM utility1_install_der_raw
        """
    ...
    df.createOrReplaceTempView("utility1_install_der_raw")

    result = spark.sql(transformSQL)

    result.write.save('/Users/kateweber/dev/ESource-Case-Study/results/utility1_install_der_bronze', format='parquet', mode='overwrite')